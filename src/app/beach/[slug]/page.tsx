...
import { BeachPhotoCarousel } from "@/components/BeachPhotoCarousel"
...
export default async function BeachDetail({
  ...
  const userLat = resolvedSearchParams?.lat ? parseFloat(resolvedSearchParams.lat as string) : null
  const userLon = resolvedSearchParams?.lon ? parseFloat(resolvedSearchParams.lon as string) : null
  ...
  if (userLat && userLon && beach.coordinates) {
    const R = 6371 // Radius of the earth in km
    const dLat = (beach.coordinates.lat - userLat) * (Math.PI / 180)
    const dLon = (beach.coordinates.lon - userLon) * (Math.PI / 180)
    const a =
      Math.sin(dLat / 2) * Math.sin(dLat / 2) +
      Math.cos(userLat * (Math.PI / 180)) * Math.cos(beach.coordinates.lat * (Math.PI / 180)) *
  ...
  const backLink = `/?${new URLSearchParams(resolvedSearchParams as any).toString()}`
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <header className="sticky top-0 z-50 border-b bg-white/80 dark:bg-slate-950/80 backdrop-blur-md dark:border-slate-800">
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          <Link href={backLink} className="flex items-center gap-2 text-slate-900 dark:text-slate-100 hover:text-rose-500 transition-colors">
          ...
          <div className="flex items-center gap-2">
            ...
            <ShareButton
              title={beach.name}
              text={`Check out ${beach.name} on BeachAtlas!`}
              url={`https://best-beachs.vercel.app/beach/${beach.slug}`}
            />
            <form action={async () => {
              "use server"
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {/* Photo Grid */}
        {/* Photo Grid (Desktop) */}
        <div className="hidden sm:grid mb-12 h-[500px] grid-cols-4 grid-rows-2 gap-2 overflow-hidden rounded-2xl">
        ...
        <div className="block sm:hidden mb-8">
          <BeachPhotoCarousel photos={beach.photos} beachName={beach.name} />
        </div>
